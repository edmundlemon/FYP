			...
			<div class="space-y-5">
				<div class="mb-4">
					...
					<input type="date" id="date"
						...
						v-model="booking.slot.date" required />
			...
			<div class="flex flex-row space-x-3 w-full">
				<div class="mb-4 w-1/2">
					...
					<input type="time" id="start_time"
						class="p-2 bg-gray-700 text-white rounded w-full transition duration-300 ease-in-out w-full rounded-md border-0 shadow-sm ring-1 ring-inset ring-gray-700 focus:ring-2 focus:ring-inset focus:ring-white"
						v-model="booking.slot.start_time" min="09:00" max="16:00" required />
				...
				<div class="mb-4 w-1/2">
					...
					<input type="time" id="end_time"
						class="p-2 bg-gray-700 text-white rounded w-full transition duration-300 ease-in-out w-full rounded-md border-0 shadow-sm ring-1 ring-inset ring-gray-700 focus:ring-2 focus:ring-inset focus:ring-white"
						v-model="booking.slot.end_time" min="09:00" max="17:00" required />
...
function bookTime() {
	...
	if (store.state.role === "student") {
		uri = `/student/reschedule/${booking.value.slot.id}`;
		console.log(uri);
	} else {
		uri = `/lecturer/reschedule/${booking.value.slot.id}`;
		console.log(uri);
	}
	axiosInstance
